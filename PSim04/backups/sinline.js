...
function update(){
    ...
    for(let i = 0; i < particles.length; i++){
        ...
        //random effect
        //what if we connect particles that are on the same height level? or x ?
        let yLevel = (Math.sin(performance.now()/300)+1)/2*can.height;
        let tolerance = 30;
        for(let j = 0; j < particles.length; j++){
            let p2 = particles[j];
            if(p == p2) continue;
            if(getDist(p.x,p.y,p2.x,p2.y) < 30 && Math.abs(p.y - yLevel) < tolerance && Math.abs(p2.y - yLevel) < tolerance){
                ctx.beginPath();
                ctx.moveTo(p.x, p.y);
                ctx.lineTo(p2.x, p2.y);
                ctx.stroke();
